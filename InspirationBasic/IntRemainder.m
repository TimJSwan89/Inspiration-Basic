...
-(int) evaluateAgainst:(EnvironmentModel *)environment {
    ...
    if ([ProgramException checkExceptionWithEnvironment:environment andIdentifier:@"IntRemainder Expression 1"])
        return 0;
    int numerator = numeratorValue;
    
    int denominatorValue = [self.expression2 evaluateAgainst:environment];
    if ([ProgramException checkExceptionWithEnvironment:environment andIdentifier:@"IntRemainder Expression 2"])
        return 0;
